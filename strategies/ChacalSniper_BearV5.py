...
class ChacalSniper_BearV5(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # BTC 5m y 1m (Optimización merge_asof)
        ...
        if inf_1m is not None and not inf_1m.empty:
            ...
            # Filtro: Solo dispara si hay cuerpo real y volumen significativo
            pump_signal = (
            ...
            # 3 de las últimas 4 velas deben ser pump
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Entrada Sniper con Switch Maestro
        short_cond = (
            (dataframe['master_bear_switch'] == 1) &
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        # Prioridad 1: DNA específico
        dna_sl = self.hyperopt_dna.get(pair, {}).get("bear_sl")
        if dna_sl:
            return dna_sl
        # Prioridad 2: Hyperopt Global
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if open_minutes < 60 or current_profit > -0.01:
            return None
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float,
        ...
        # LEVERAGE DINÁMICO ATR-BASED
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        if dataframe is None or dataframe.empty:
            return 5.0
        
        # Rank de ATR de las últimas 168 horas (1 semana)
        # Usamos el pool de BTC como proxy de volatilidad del mercado
        atr_rank = dataframe['btc_atr'].rolling(2016).rank(pct=True).iloc[-1] # 2016 velas de 5m = 1 semana
        
        if atr_rank < 0.3: # Calma relativa
            return 7.0
        elif atr_rank < 0.6: # Volatilidad normal
            return 5.0
        else: # Estrés de mercado
            return 3.0
