...
- Mejoras:
    ...
    5. Kill Switch Institucional (RSI 1m > 85 o 48h timeout).
"""
import numpy as np
import pandas as pd
...
class ChacalVolumeHunter_V5(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # INFORMACION MICRO (1m) para Kill Switch
        ...
        if inf_1m is not None and not inf_1m.empty:
            ...
            dataframe = pd.merge_asof(
                ...
                inf_1m[['date', 'rsi_1m']].sort_values('date'),
                on='date',
                direction='backward'
            )
        else:
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float,
        ...
        if atr_rank < 0.3: return 10.0
        elif atr_rank < 0.7: return 7.0
        else: return 3.0
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # 2. RSI 1m Extremo (Salida de seguridad en paradas bruscas)
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        if dataframe is not None and not dataframe.empty:
            last_rsi_1m = dataframe.iloc[-1].get('rsi_1m', 0)
            if last_rsi_1m > 85:
