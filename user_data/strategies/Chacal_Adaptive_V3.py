...
class Chacal_Adaptive_V3(IStrategy):
    ...
    alerta_threshold  = DecimalParameter(0.20, 0.50, default=0.30, space='buy', load=True)
    rebelde_threshold = DecimalParameter(0.40, 0.70, default=0.50, space='buy', load=True)
    pivot_threshold   = DecimalParameter(0.45, 0.85, default=0.60, space='buy', load=True)

    # --- DINÁMICA DE CHOQUE (PEGASO 3.4) ---
    inertia_window = IntParameter(1, 5, default=2, space='buy', load=True)
    div_nitro_mult = DecimalParameter(1.5, 3.0, default=2.0, space='buy', load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # --- HARDENING: TENDENCIA BTC ---
        # Si BTC_EMA_5m está subiendo, bloqueamos cortos rebeldes
        btc_ema_5 = ta.EMA(dataframe['btc_5m_close'], timeperiod=5)
        dataframe['btc_is_rising'] = (dataframe['btc_5m_close'] > btc_ema_5) & (btc_ema_5 > btc_ema_5.shift(2))
        ...
        # --- DINÁMICA DE CHOQUE: INERCIA PARAMÉTRICA ---
        # El bot decide cuántas velas de confirmación necesita
        dataframe['pivot_confirmed'] = (dataframe['state_pair'] == 3).rolling(int(self.inertia_window.value)).sum() >= (int(self.inertia_window.value) * 0.8)
        
        # --- NITRO DE DIVERGENCIA ---
        # Si la divergencia es brutal, ignoramos la inercia (disparo instantáneo)
        div_nitro = dataframe['divergence_btc'].abs() > (div_floor * float(self.div_nitro_mult.value))
        dataframe['pivot_trigger'] = dataframe['pivot_confirmed'] | div_nitro
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # RUTA V3: SHORT PIVOT (PEGASO 3.4 AGRESIVO)
        pivot_short = (
            (dataframe['market_mode'] <= 0) &
            (dataframe['pivot_trigger']) & 
            (dataframe['pivot_trigger'].shift(1) == False) & 
            (~dataframe['btc_is_rising']) & 
